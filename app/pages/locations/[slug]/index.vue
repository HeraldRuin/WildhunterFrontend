...
const cachedLocation = computed(() => {
  return homeLocations.value?.find(item => item.slug === locationSlug.value) ?? null
})

const cachedLocationName = computed(() => cachedLocation.value?.title ?? '')
...
const { filters, clearPersistedFilters } = usePersistedSearchFilters(
  `location-search-filters-${locationSlug.value}`,
  () => ({
    ...DEFAULT_SEARCH_FILTERS,
...
const { data: locationHotels, pending: hotelsPending } = useAsyncData(
  () => `location-hotels-${route.params.slug}`,
  async () => {
    ...
    try {
      return await locationApi.getLocationHotelItems(locationSlug.value)
    }
    catch {
      return [] as OfferItem[]
    }
...
// Поиск по удобствам (/hotels/search) принимает числовой location_id,
// поэтому id локации резолвится по slug из общего списка локаций.
const { data: resolvedLocation, pending: locationPending } = useAsyncData(
  () => `location-entry-${route.params.slug}`,
  async () => {
    if (!locationSlug.value) {
      return null
    }
    ...
    try {
      ...
      return locations.find(item => item.slug === locationSlug.value) ?? null
    }
    catch {
      return null
    }
  ...
  {
    lazy: true,
    default: () => null as SearchLocation | null,
    watch: [locationSlug],
    getCachedData: (key, nuxtApp) => getCachedPageData(key, nuxtApp),
  },
)

const locationId = computed(() => cachedLocation.value?.id ?? resolvedLocation.value?.id ?? null)
...
watch(
  () => [locationId.value, locationPending.value, filters.value.amenities] as const,
  async ([id, locationLoading, amenities]) => {
    if (!amenities.length) {
    ...
    if (id == null) {
      ...
      amenitiesPending.value = locationLoading
      return
    }
      ...
      <div class="container location-page__results-inner">
        ...
        >
          <div class="location-page__toolbar">
            ...
            <NuxtLink
              v-if="countReady"
              :to="getLocationMapPath(locationSlug)"
