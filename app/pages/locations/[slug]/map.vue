...
const cachedLocationName = computed(() => {
  return homeLocations.value?.find(item => item.slug === locationSlug.value)?.title ?? ''
})
...
const { data: fetchedLocationName } = useAsyncData(
  () => `location-name-${route.params.slug}`,
  async () => {
    ...
    try {
      ...
      const match = locations.find(item => item.slug === locationSlug.value)
...
const breadcrumbs = computed<BreadcrumbItem[]>(() => {
  ...
  return [
    ...
    {
      label: name,
      to: locationSlug.value ? getLocationPath(locationSlug.value) : undefined,
    },
    { label: 'Карта' },
  ]
})
