...
export function normalizeUserProfile(data: unknown, uploadsOrigin = ''): ProfileUser {
  ...
  return {
    ...
    hunter_billet_issuing_authority: String(source.hunter_billet_issuing_authority ?? '').trim(),
    hunter_billet_rf_subject: String(source.hunter_billet_rf_subject ?? '').trim(),
    hunter_billet_issue_date: formatBirthdayDisplay(source.hunter_billet_issue_date),
    identity_document: String(source.identity_document ?? '').trim(),
